...
public class DefaultRealmPullResultHandler
    ...
    private static final String COULD_NOT_UPDATE_REALM = "Could not update Realm {}";
    private static final String PULL_TASK_NOT_CONFIGURED_UPDATE = "PullTask not configured for update";
    private static final String SUCCESSFULLY_UPDATED = "{} successfully updated";
    private static final String COULD_NOT_PROPAGATE_REALM = "Could not propagate Realm {}";
    ...
    public DefaultRealmPullResultHandler(
            ...
            // Parametri della superclasse iniettati per garantire il funzionamento di @InjectMocks
            final org.apache.syncope.core.persistence.api.dao.RealmDAO realmDAO,
            final org.apache.syncope.core.persistence.api.dao.RealmSearchDAO realmSearchDAO,
            final org.apache.syncope.core.provisioning.api.data.RealmDataBinder binder,
            final org.apache.syncope.core.provisioning.api.propagation.PropagationManager propagationManager,
            final org.apache.syncope.core.provisioning.api.propagation.PropagationTaskExecutor taskExecutor,
            final org.apache.syncope.core.provisioning.api.notification.NotificationManager notificationManager,
            final org.apache.syncope.core.provisioning.api.AuditManager auditManager,
            final org.apache.syncope.core.spring.security.SecurityProperties securityProperties) {

        // Campi diretti della classe
        ...
        // Assegnazione manuale ai campi protetti ereditati
    ...
    protected OpEvent.Outcome create(
        ...
        try {
            ...
            LOG.error(COULD_NOT_PROPAGATE_REALM, delta.getUid().getUidValue(), e);
            output = e;
    ...
    protected OpEvent.Outcome update(final SyncDelta delta, final Realm realm, final boolean inLink)
        ...
        if (!profile.getTask().isPerformUpdate()) {
            LOG.debug(PULL_TASK_NOT_CONFIGURED_UPDATE);
        ...
        try {
            ...
            LOG.debug(SUCCESSFULLY_UPDATED, updated);
        } catch (PropagationException e) {
            LOG.error(COULD_NOT_PROPAGATE_REALM, delta.getUid().getUidValue(), e);
            output = e;
            resultStatus = OpEvent.Outcome.FAILURE;
            ...
            result.setStatus(ProvisioningReport.Status.FAILURE);
            result.setMessage(ExceptionUtils.getRootCauseMessage(e));
            LOG.error(COULD_NOT_UPDATE_REALM, delta.getUid().getUidValue(), e);
            output = e;
            resultStatus = OpEvent.Outcome.FAILURE;
        }
    ...
    protected OpEvent.Outcome deprovision(final SyncDelta delta, final Realm realm, final boolean unlink)
        ...
        try {
            ...
            LOG.debug(SUCCESSFULLY_UPDATED, realm);
        } catch (PropagationException e) {
            LOG.error(COULD_NOT_PROPAGATE_REALM, delta.getUid().getUidValue(), e);
            output = e;
            resultStatus = OpEvent.Outcome.FAILURE;
    ...
    protected OpEvent.Outcome delete(final SyncDelta delta, final Realm realm) {
        ...
        try {
            ...
            if (!profile.isDryRun()) {
                ...
                resultStatus = performDelete(delta, realm, before, result);
            }
    ...
    private OpEvent.Outcome performDelete(
            final SyncDelta delta,
            final Realm realm,
        ...
        try {
            switch (delta.getDeltaType()) {
                case CREATE, UPDATE, CREATE_OR_UPDATE:
                    if (realms.isEmpty()) {
                        result = handleUnmatching(finalDelta, orgUnit);
                    } else {
                        result = handleMatching(finalDelta, realm);
                    }
                    break;

                case DELETE:
                    result = delete(finalDelta, realm);
                    break;
    ...
    private OpEvent.Outcome handleUnmatching(final SyncDelta finalDelta, final OrgUnit orgUnit) throws JobExecutionException {
        switch (profile.getTask().getUnmatchingRule()) {
            case ASSIGN:
                return assign(finalDelta, orgUnit);
            case PROVISION:
                return provision(finalDelta, orgUnit);
            case IGNORE:
                return ignore(finalDelta, false);
            default:
                return OpEvent.Outcome.SUCCESS;
        }
    }

    private OpEvent.Outcome handleMatching(final SyncDelta finalDelta, final Realm realm) throws JobExecutionException {
        switch (profile.getTask().getMatchingRule()) {
            case UPDATE:
                return update(finalDelta, realm, false);
            case DEPROVISION:
                return deprovision(finalDelta, realm, false);
            case UNASSIGN:
                return deprovision(finalDelta, realm, true);
            case LINK:
                return link(finalDelta, realm, false);
            case UNLINK:
                return link(finalDelta, realm, true);
            case IGNORE:
                return ignore(finalDelta, true);
            default:
                return OpEvent.Outcome.SUCCESS;
        }
    }
