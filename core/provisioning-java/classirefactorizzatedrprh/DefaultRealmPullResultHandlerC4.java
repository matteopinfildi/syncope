...
public class DefaultRealmPullResultHandler
    ...
    private static final String MSG_PULL_TASK_NOT_CONFIGURED_FOR_UPDATE = "PullTask not configured for update";
    private static final String MSG_PULL_TASK_NOT_CONFIGURED_FOR_CREATE = "PullTask not configured for create";
    ...
    private OpEvent.Outcome createRealm(
            final SyncDelta delta,
            final OrgUnit orgUnit,
            final UnmatchingRule unmatchingRule) throws JobExecutionException {

        if (!profile.getTask().isPerformCreate()) {
            LOG.debug(MSG_PULL_TASK_NOT_CONFIGURED_FOR_CREATE);
            end(Optional.empty(),
                    UnmatchingRule.toOp(unmatchingRule),
    ...
    protected OpEvent.Outcome update(final SyncDelta delta, final Realm realm, final boolean inLink)
        ...
        if (!profile.getTask().isPerformUpdate()) {
            LOG.debug(MSG_PULL_TASK_NOT_CONFIGURED_FOR_UPDATE);
    ...
    protected OpEvent.Outcome doHandle(final SyncDelta delta, final OrgUnit orgUnit) throws JobExecutionException {
        ...
        return processDelta(finalDelta, orgUnit, realms, realm);
    }

    private OpEvent.Outcome processDelta(
            final SyncDelta delta,
            final OrgUnit orgUnit,
        ...
        try {
            switch (delta.getDeltaType()) {
                ...
                case CREATE_OR_UPDATE:
                    return handleCreateOrUpdate(delta, orgUnit, realms, realm);

                case DELETE:
                    return delete(delta, realm);
    ...
    private OpEvent.Outcome handleCreateOrUpdate(
            final SyncDelta delta,
            final OrgUnit orgUnit,
            final List<Realm> realms,
            final Realm realm) throws JobExecutionException {
        ...
        if (realms.isEmpty()) {
            switch (profile.getTask().getUnmatchingRule()) {
                case ASSIGN:
                    return assign(delta, orgUnit);
                case PROVISION:
                    return provision(delta, orgUnit);
                case IGNORE:
                    return ignore(delta, false);
                default:
                    return OpEvent.Outcome.SUCCESS;
            }
        }
        ...
        switch (profile.getTask().getMatchingRule()) {
            case UPDATE:
                return update(delta, realm, false);
            case DEPROVISION:
                return deprovision(delta, realm, false);
            case UNASSIGN:
                return deprovision(delta, realm, true);
            case LINK:
                return link(delta, realm, false);
            case UNLINK:
                return link(delta, realm, true);
            case IGNORE:
                return ignore(delta, true);
            default:
                return OpEvent.Outcome.SUCCESS;
        }
    }
